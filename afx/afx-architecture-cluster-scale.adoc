...
* Node removals in NetApp AFX also don't require much – if any – manual intervention. When a node is tagged for removal, ONTAP automatically moves volumes across nodes (again, without copying) to evacuate the nodes being removed. And because there are no disks owned by the nodes, there is no need to reinitialize disks after removing nodes. This makes nodes in AFX modular in nature and easy to scale up or down. For the procedure, see link:afx-remove-nodes.html[How to remove AFX nodes].
...
For maximum cluster size information, see the link:afx-limits.html[Limits] page.
