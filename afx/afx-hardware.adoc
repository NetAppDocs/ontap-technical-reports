...
| Cisco Nexus 9364D-GX2A | 32 nodes and 15 shelves (32 + 15×2 = 62 ports) | Not supported (96 node ports exceed 62 usable ports)
| Cisco Nexus 9808 | 32 nodes and up to 25 shelves (platform shelf maximum) | 32 nodes and up to 25 shelves (platform shelf maximum)
|===

==== Maximum node count per switch

If the cluster is smaller than 32 nodes, the following combinations fill each switch's usable ports while keeping even node counts (HA pairs) and a minimum of 2 shelves. Remaining ports are used for additional shelves. Shelf count cannot exceed the platform maximum (25 as of ONTAP 9.19.1).

[options="header"]
|===
| Switch | AFX 1K (max nodes + shelves) | AFX 2K (max nodes + shelves)
| Cisco Nexus 9332D-GX2B | 26 nodes + 2 shelves | 8 nodes + 3 shelves
| Cisco Nexus 9364D-GX2A | 32 nodes + 15 shelves | 18 nodes + 4 shelves
| Cisco Nexus 9808 | 32 nodes + 25 shelves | 32 nodes + 25 shelves
|===
